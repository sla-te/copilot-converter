...
Parse `${input:requirements}` for `--strategy` flag. Use `parallel-systems` as default if not specified.

### 3. Parse target description

Extract the target description from `${input:requirements}` (everything before the flags). This is referenced as `$TARGET` in prompts below.
