...
Parse `${input:requirements}` for `--methodology` and `--complexity` flags. Use defaults if not specified.

### 3. Parse feature description

Extract the feature description from `${input:requirements}` (everything before the flags). This is referenced as `$FEATURE` in prompts below.
