...
description: "def product_list(request): products = Product.objects.all() return JsonResponse({'products': list(products.values())})"
...
Prompt: "Implement MINIMAL code to make these failing tests pass: ${input:requirements}. Follow TDD green phase principles:
...
Tests to make pass: ${input:requirements}
